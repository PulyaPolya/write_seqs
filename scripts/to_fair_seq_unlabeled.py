"""
Output of write_unlabeled_seqs looks like this:
```
./
1.csv 2.csv ...
```

Input to the fairseq binarizer should look like this:

```
dict.input.txt	  midi_test.txt
```

We also write files `metadata_test.csv` containing the 
...
def main():
    ...
    csv_paths = [input_dir / f for f in os.listdir(input_dir) if f.endswith(".csv")]
    split = "test"
