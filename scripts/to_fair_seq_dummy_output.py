"""
For debugging. Like to_fair_seq.py, but produces dummy targets uniformly distributed
over `--targets`
"""
...
def main():
    ...
    for split in ("train", "test", "valid"):
        n_examples_remaining = args.n_examples
        csv_paths = [
        ...
        for csv_path in csv_paths:
            ...
            csv_contents = csv_contents.iloc[:n_examples_remaining]
            if n_examples_remaining is not None:
                n_examples_remaining -= len(csv_contents)
            if len(csv_contents):
                ...
                with open(output_dir / f"targets_{split}.txt", "a") as appendf:
                    for row in csv_contents["chord_tone"]:
                        n_elems = len(row.split())
                        dummy_targets = random.choices(args.targets, k=n_elems)
                        appendf.write(" ".join(dummy_targets))
                        appendf.write("\n")

            if n_examples_remaining is not None and n_examples_remaining <= 0:
                break
